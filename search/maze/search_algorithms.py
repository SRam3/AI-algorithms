...
class GreedyBestFirstSearchFrontier:
    ...
    def add(self, node):
        ...
        self.frontier.append((distance, node))
    ...
    def remove(self):
        if self.empty():
            ...
            # Save the node with the lowest distance from the goal
            node = min(self.frontier, key=lambda t: t[0])[1]
            # Save all the nodes besides the one with the lowest distance from the goal
            self.frontier = [x for x in self.frontier if x[1] != node]
            return node
